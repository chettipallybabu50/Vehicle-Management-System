    ...
    try{
        ...
        if(CompanyId){
            const [results] = await pool.query(
                `SELECT  name As Company_name, Basement_Name, BasementID, four_wheeler_slots_reserved, two_wheeler_slots_reserved,
             basement_reserved_slots FROM tenant_basement_details 
             WHERE TenantID = ?`,
                [CompanyId]
            );
            const companywisevehicledata = results as any[];
            const companyName = companywisevehicledata[0].Company_name;
            const workbook = new ExcelJS.Workbook();
            const worksheet = workbook.addWorksheet(`${companyName}-Basement`);
            ...
            companywisevehicledata.forEach((row) => {
                worksheet.addRow({
            ...
            if (companywisevehicledata.length > 0) {
                worksheet.addRow({
                    serial_number: "",
            ...
            const fileName = `${companyName}_Company_report.xlsx`;
